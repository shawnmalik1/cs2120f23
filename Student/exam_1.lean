...
| (Expr.top_exp ⊤), i => (eval_top_expr ⊤) (eval_expr ⊤ i)
| (Expr.top_exp ⊥), i => (eval_top_expr ⊤) (eval_expr ⊤ i)
...
| Expr.top_exp _ ⊤ => max_variable_index ⊤
| Expr.bottom_exp _ ⊥ => max_variable_index ⊥
...
| mk : Bread → (Cheese ⊕ Spread) → Sandwich 
...
Sandwich.mk Bread.wheat Spread.jam
...
--wrote the top_exp and bottom_exp and defined all the methods
--but it is giving a small error that I don't know how to
--fix, so is not 100% implemented.
