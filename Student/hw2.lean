...
/-
No, they would not be the same answer. This is because of the order glue_funs 
takes in the arguments and computes them. It does g (f a), meaning the first example
would first take (square 5), which is 25, then double it, giving 50.

The second example would firs take (double 5) which is 10, then square it, giving 100.
-/